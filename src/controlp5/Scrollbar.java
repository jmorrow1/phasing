...
/**
 * A scrollbar that works with the ControlP5 library.
 * Terminology:
 * 	   Track: The thing that extends the entire length of the scrollbar. The thing in which the scroller moves.
 *     Scroller: The thing that moves along the track.
 *     Tick mark: An abstract unit that cooresponds to the scroller's location along the track.
 *                It's easier for the outside world to interface with tick marks as opposed to geometrical data when doing logic with the state of the scrollbar.
 * @author James Morrow
 *
 */
public class Scrollbar extends Controller<Scrollbar> {
     ...
     * @param ticksPerScroller The size of the scroller (the thing the user can move along the track) in terms of tick marks
     * @param ticksPerTrack The size of the track (the thing that the scroller moves along) in terms of tick marks
     */
    ...
    /**
     * Updates the state of the geometrical data.
    ...
    /**
     * Updates the state of the tick mark cooresponding to the lowest point of the scroller.
     * 
     */
    ...
    /**
     * Checks if the mouse pointer (which is the mouse location, relative to the controller) is within the area of the scroller.
     * @return True, if the mouse pointer is within the area of the scroller. False otherwise.
     */
    private boolean touchesScroller() {
    	if (getWidth() >= getHeight()) {
    		return scrollerLow <= getPointer().x() && getPointer().x() <= scrollerHigh &&
    				0 <= getPointer().y() && getPointer().y() <= getHeight();
    	}
    	else {
    		return 0 <= getPointer().x() && getPointer().x() <= getWidth() && 
    				scrollerLow <= getPointer().y() && getPointer().y() <= scrollerHigh;
    	}
    }
    ...
    /**
     * A hack to make it possible for mouse scroll events to adjust the state of the scrollbar even when the mouse is not within the area of the controller.
     * Without this, the Scrollbar can only receive mouse scroll events when the mouse is touching the controller. 
	...
	/**
	 * 
	 * @return The number of tick marks associated with the whole length of the scrollbar.
	 */
	...
	/**
	 * Sets the number of tick marks associated with the whole length of the scrollbar.
	 * @param ticksPerTrack
	 */
