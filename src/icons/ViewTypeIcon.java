...
public class ViewTypeIcon implements Icon {
    ...
    public void draw(float x, float y, float radius, PhasesPApplet pa) {
        ...
        pa.fill(0);
        for (int i = 0; i < 12; i++) {
            pa.ellipse(x + PApplet.cos(theta) * markerRadius, y + PApplet.sin(theta) * markerRadius, dotRadius,
                    dotRadius);
            theta += dTheta;
        }
        ...
        case PHASE_SHIFTER:
            ...
            /*
             * pa.stroke(pa.getColor2()); pa.drawSineWave(x, y, 1.5f*radius,
             * 0.5f*radius, 0); pa.stroke(pa.getColor1()); pa.drawSineWave(x, y,
             * 1.5f*radius, 0.5f*radius, pa.HALF_PI);
             */
            break;
        case LIVE_SCORER:
            past(x, y, handLength, pa);
