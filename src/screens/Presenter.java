...
public class Presenter extends Screen implements ViewVariableInfo, PhraseReaderListener {
    ...
    // unlock sequences (in terms of minutes to unlock thing 1, minutes to
    // unlock thing 2, etc.)
     ...
     * 
     * @param playerInfo
     *            The PlayerInfo object to initialize each view with.
     */
     ...
     * @param headAngle
     *            The angle (in radians) of the heading.
     * @param name
     *            The name of the button (which ControlP5 will use for
     *            callbacks).
     * @return
     */
    private Button consDirectionalButton(float x1, float y1, int radius, float headAngle, String name) {
        Button b = cp5.addButton(name).setPosition(x1, y1).setSize(2 * radius, 2 * radius)
                .setView(new TriangleButtonView(headAngle, 0.75f * pa.TWO_PI)).plugTo(this);
    ...
    /**
     * Gives where the x-coordinate of a directional button that's associated
     * with an icon at a given index (whether or not there's actually an icon
     * there) would be.
     * 
     * @param index
    ...
    /**
     * This method can be triggered by ControlP5 (as a callback) and by the
     * presenter itself.
     * 
     * Moves the activeVar up, if it can. Changes the icon being displayed at
     * the activeIconIndex accordingly. Checks to see if the view type needs to
     * be changes and changes it if it does.
     */
    public void iconUp() {
     ...
     * Presenter itself.
     * 
     * Moves the activeVar down, if it can. Changes the icon being displayed at
     * the activeIconIndex accordingly. Checks to see if the view type needs to
     * be changes and changes it if it does.
     */
    ...
    /**
     * Moves the activeIconIndex right. Messages the directional buttons to
     * reposition themselves according to the new activeIconIndex.
     */
    ...
    /**
     * Moves the activeIconIndex left. Messages the directional buttons to
     * reposition themselves according to the new activeIconIndex.
     */
    ...
    /**
     * Checks if the view should change type. If it should, the method changes
     * the view type accordingly.
     */
    ...
    /**
     * Checks if the current phrase should change orientation. If it should, the
     * method changes it.
     */
    /*
     * private void checkPhraseOrientation() { if
     * (phraseOrientationShouldChange()) { phraseIsReversed = !phraseIsReversed;
     * musicianView.wakeUp(0, 0); liveScorerView.wakeUp(0, 0);
     * phaseShifterView.wakeUp(0, 0); setupPlayback(); } }
     */
     ...
     */
    /*
     * private boolean phraseOrientationShouldChange() { return
     * (phraseIsReversed && liveScorerView.orientationMode.toInt() ==
     * NON_REVERSED) || (!phraseIsReversed &&
     * liveScorerView.orientationMode.toInt() == REVERSED); }
     */

    /**
     * Checks if the instrument sound should change. If it should, the method
     * changes it.
     */
    ...
    /**
     * Updates variables related to tracking the amount of time the program has
     * been executing under certain conditions.
     */
    private void updateTime() {
        ...
        switch (viewType.toInt()) {
        case MUSICIAN:
            pa.playerInfo.minutesSpentWithMusician += changeInMinutes;
            break;
        case PHASE_SHIFTER:
            pa.playerInfo.minutesSpentWithPhaseShifter += changeInMinutes;
            break;
        case LIVE_SCORER:
            pa.playerInfo.minutesSpentWithLiveScorer += changeInMinutes;
            break;
        }
    }
    ...
    /**
     * Gives the center x-coordinate of the icon at the given index (whether or
     * not an icon actually exists there).
     * 
     * @param index
    ...
    /**
     * If the given (x,y) position lies within the area in which an icon is
     * being displayed, the index of that icon is returned. If the given (x,y)
     * position does not lie within the area in which an icon is being
     * displayed, -1 is returned.
     * 
     * The hit box is actually a bit taller than the space the icon takes up,
     * which is on purpose.
     * 
     * @param x
    ...
    private int iconIndexTouches(int x, int y) {
        ...
        if (iconStartX1 - iconRadius <= x && x < iconEndX && iconStartY1 - iconRadius <= y
                && y <= iconStartY1 + 3f * iconRadius) {
    ...
    /**
     * 
     * @return The position of player1 along the duration of the phrase it is
     *         playing.
     */
    private float computeNotept1() {
        if (pa.currentPhrase.getTotalDuration() == 0)
            return 0;
        return PApplet.map(player1.getTickPosition(), 0, player1.getTickLength(), 0,
                pa.currentPhrase.getTotalDuration());
    }

    /**
     * 
     * @return The position of player2 along the duration of the phrase it is
     *         playing.
     */
    private float computeNotept2() {
        if (pa.currentPhrase.getTotalDuration() == 0)
            return 0;
        return PApplet.map(player2.getTickPosition(), 0, player2.getTickLength(), 0,
                pa.currentPhrase.getTotalDuration());
    }

    /**
     * First, it computes information relating to musical timing, such as the
     * duration a player has traveled since last frame. Then it sends that
     * information to the active view and tells the view to update and draw
     * itself.
     */
    ...
    /**
     * First, it checks if a new icons has been unlocked for the active view
     * type. If there's been a new unlock, it calls setupIconLists(), which
     * refreshes the state of the icon lists. If there's been a new unlock, it
     * updates the player info save file.
     */
    private void checkUnlocks() {
        boolean unlockedSomething = false;

        if (viewType.toInt() == MUSICIAN && pa.playerInfo.nextMusicianUnlockIndex < musicianUnlockSeq.length
    ...
    /**
     * Adds all the unlocked view type icons to the container called
     * "iconLists", which stores all the icons that are currently available to
     * the player.
     */
    ...
    /**
     * Adds all the unlocked icons relating to the current view type + all the
     * unlocked view type icons.
     */
    ...
    // TODO Pos. improvement: Implement unlock sequences by interpreting a text
    // file that defines the unlock sequences

    /**
     * 
     * @param name
     *            The String identifier for a type of icon related to the
     *            current view type. All of these kinds of names are given in
     *            views.ViewVariableInfo.java.
     * @return The number of icons that are available (unlocked) of the icon
     *         type associated with the given identifier.
     */
    private int getIconAvailability(String name) {
        ...
        default:
            switch (viewType.toInt()) {
            case MUSICIAN:
                return getMusicianIconAvailability(name);
            case PHASE_SHIFTER:
                return getPhaseShifterIconAvailability(name);
            case LIVE_SCORER:
                return getLiveScorerIconAvailability(name);
            default:
                return 0;
            }
        }
    ...
    /**
     * 
     * @return True if the conditions are met for the PhaseShifter icon to be
    ...
    /**
     * 
     * @return True if the conditions are met for the LiveScorer icon to be
     *         unlocked, false otherwise.
     */
     ...
     * 
     * @param name
     *            The String identifier for a type of icon related to the view
     *            type called Musician. All of these kinds of names are given in
     *            views.ViewVariableInfo.java.
     * @return The number of icons that are available (unlocked) of the icon
     *         type associated with the given identifier.
     */
     ...
     * @param name
     *            The String identifier for a type of icon related to the view
     *            type called PhaseShifter. All of these kinds of names are
     *            given in views.ViewVariableInfo.java.
     * @return The number of icons that are available (unlocked) of the icon
     *         type associated with the given identifier.
     */
     ...
     * @param name
     *            The String identifier for a type of icon related to the view
     *            type called LiveScorer. All of these kinds of names are given
     *            in views.ViewVariableInfo.java.
     * @return The number of icons that are available (unlocked) of the icon
     *         type associated with the given identifier.
     */
