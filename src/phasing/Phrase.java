 ...
 * 
 * The first representation models a musical phrase as a sequence of notes. In
 * this case a note is a pitch, a dynamic, a duration, an articulation, and a
 * pan value. The class uses 5 parallel arrays to do this: one for pitches, one
 * for dynamics, etc. This representation's purpose is to directly interface
 * with the soundcipher library. From the perspective of the outside world, this
 * representation's data is read-only.
 * 
 * The second representation models a musical phrase as a matrix with 5 rows
 * with an arbitrary number of columns. Again there are a similar set of 5
 * parameters (pitch, dynamic, etc.), and again parallel arrays are used. The
 * difference is what each column represents. In the 1st representation, each
 * column represented a note. In this representation, each column represents a
 * more abstract thing: a cell. Owing to this difference, instead of an array of
 * durations there is an array of "cell types". A cell type can be one of 3
 * things: a "note start", a "note sustain", or a "rest". A "note sustain" cell
 * allows notes to exist over several consecutive cells. This representation's
 * purpose is to make it simple for the Editor to interface with a Phrase (this
 * representation models a musical phrase as a grid and so does the Editor).
 * From the perspective of the outside world, this representation's data is
...
public class Phrase implements JSONable {
     ...
     * 
     * @param gridPitches
     *            The sequence of MIDI pitch values
     * @param gridDynamics
     *            The sequence of dynamic values
     * @param cellTypes
     *            The sequence of cell types
     */
    public Phrase(float[] gridPitches, float[] gridDynamics, int[] cellTypes, String scaleClassName,
            String scaleRootName) {
     ...
     *            The sequence of cell types
     * @param defaultArt
     *            The articulation value to give each note
     * @param defaultPan
     *            The pan value to give each note
     ...
     *            The sequence of cell types
     * @param gridArts
     *            The sequence of articulation values
     * @param gridPans
     *            The sequence of pan value
     * @param defaultArt
     * @param defaultPan
     * @param scaleClassName
     * @param scaleRootName
     ...
     * 
     * @param json
     *            A JSONObject representing the phrase.
     */
    ...
    /**
     * Adds this object's phrase information to an SCScore object (a soundcipher
     * utility).
     * 
     * @param score
     *            The SCScore object
     * @param startBeat
     *            The beat on which to start
     * @param channel
     *            The MIDI channel
     * @param instrument
     *            The MIDI instrument
    ...
    /**
     * Updates the data fields associated with the phrase's soundcipher
     * representation to put them in agreement with the current state of the
     * phrase. For efficiency reasons, often the phrase's soundcipher
     * representation will be out of date. In these cases, a variable called
     * "scArraysUpToDate" will be set to false. This indicates that if some
     * other object requests soundcipher data or forces phrase to use
     * soundcipher data, then the data fields associated with the soundcipher
     * data will have to be updated, which is what this helper method is for.
     */
    ...
    /**
     * Searches the phrase and returns the lowest value MIDI pitch in the
     * phrase. Rests don't count as having a pitch, so they're excluded from the
     * search.
     * 
    ...
    /**
     * Searches the phrase and returns the greatest value MIDI pitch in the
     * phrase. Rests don't count as having a pitch, so they're excluded from the
     * search.
     * 
     ...
     * 
     * @param scIndex
     *            Index to the soundcipher pitches.
     ...
     * 
     * @param i
     *            The index to the phrase's soundcipher representation
     ...
     * @param i
     *            The index to the phrase's soundcipher representation
     * @return The articulation value [a multiplier to duration] at the given
     *         index
     */
    ...
    /**
     * It looks up the note at the given scIndex. Then it looks up the start
     * time of that note. It compares that start time to the total duration of
     * the phrase and returns that ratio.
     * 
     * @param scIndex
     *            The index of the desired note.
     * @return The ratio between the note's start time and the total duration of
     *         the phrase.
     */
     ...
     * 
     * @param i
     *            The index to the phrase's grid representation
     * @param pitch
    ...
    /**
     * Sets the cell type value. If successful, it returns true. For some
     * inputs, the method will not rewrite the cell type value and will return
     * false. For example, it does not make sense to put a note sustain after a
     * rest. A note cannot be sustained if it hasn't been started. Or if the
     * value given for the cellType is not valid, the method will return false.
     * 
     * @param i
     *            The index to the phrase's grid representation
     * @param cellType
     *            The value of the type of cell
    ...
    public static Phrase reverse(Phrase phrase) {
        ...
        return new Phrase(gridPitches, gridDynamics, cellTypes, gridArts, gridPans, defaultArt, defaultPan,
                scaleClassName, scaleRootName);
    }

    /**
     * Takes an array of cell types and reads it in backwards order, returning a
     * new array of cell types.
     * 
     * @param in
     *            The array of cell types.
     ...
     * 
     * @param code
     *            The MIDI pitch value
     * @param useSharps
     *            Whether to use sharps or flats in the string output
