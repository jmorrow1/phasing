 ...
 * data that determines what the player has and hasn't unlocked. --> the index
 * of the phrase the current phrase derives from. --> the last saved state of
 * view options --> the last saved bpm1 value --> the last saved phase
 * difference value
 * 
 * @author James Morrow
 *
...
public class PlayerInfo {
    ...
    public PlayerInfo(float minutesSpentWithMusician, float minutesSpentWithPhaseShifter,
            float minutesSpentWithLiveScorer, int nextMusicianUnlockIndex, int nextPhaseShifterUnlockIndex,
            int nextLiveScorerUnlockIndex, float minutesSpentWithEditor, int nextEditorUnlockIndex) {
    ...
    public PlayerInfo(JSONObject json) {
        this(json.getFloat("minutesSpentWithMusician", 0), json.getFloat("minutesSpentWithPhaseShifter", 0),
                json.getFloat("minutesSpentWithLiveScorer", 0), json.getInt("nextMusicianUnlockIndex", 0),
                json.getInt("nextPhaseShifterUnlockIndex", 0), json.getInt("nextLiveScorerUnlockIndex", 0),
                json.getFloat("minutesSpentWithEditor", 0), json.getInt("nextEditorUnlockIndex", 0));
