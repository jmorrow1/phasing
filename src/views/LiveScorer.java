...
/**
 * The LiveScorer View type. It listens for note events and every time it
 * receives one, it adds a new note to a plot of pitch/time.
 * 
 * @author James Morrow
 *
 */
public class LiveScorer extends View {
    ...
    // used in calculating where to spawn new points when scoreMode is
    // MOVE_SPAWN_POINT:
    ...
    // this helps make it such that a quarter note is drawn as a circle when the
    // stroke cap is round:
    ...
    // public ModInt orientationMode = new ModInt(0, numOrientationModes,
    // orientationModeName);
    ...
    /**
     * Constructs a LiveScorer whose option values are taken from the another
     * LiveScorer.
     * 
     * @param ls
     *            The LiveScorer this one derives its option values from.
     * @param viewBox
     *            The area in which to draw.
     * @param opacity
     *            The opacity of notes.
    ...
    /**
     * Copies the given LiveScorer object's option values into this LiveScorer
     * object's option variables.
     * 
     * @param ls
     *            The given LiveScorer.
     */
    ...
    private void initFadeRate() {
        ...
        fadeRate = 1.69139f * PApplet.pow(10f, -14f) * PApplet.pow(pa.width, 4f)
                ...
                + 1.81626f * PApplet.pow(10f, -7f) * PApplet.pow(pa.width, 2f) - 0.000163378f * pa.width + 0.0642531f;
    }
    ...
    /**
     * Initializes an array of possible y-values for notes, so they don't have
     * to be constantly recomputed. These values are used when
     * (scoreMode.toInt() == MOVE_NOTES) but not when (scoreMode.toInt() ==
     * MOVE_SPAWN_POINT).
     */
    ...
    /**
     * Initializes variables for that define the boundaries of the note spawn
     * point, which depends on the scoreMode. Then initializes the spawn point.
     */
    ...
    /**
     * Callback from the PhraseReader. When the PhraseReader reads a new note it
     * calls this method to plot that note.
     * 
     * @param reader
     *            The PhraseReader invoking the callback.
     */
    private void plotNote(PhraseReader reader) {
        ...
        if (!pa.currentPhrase.isRest(noteIndex)) {
            ...
            if (sineWave.toInt() == IS_SINE_WAVE && !pa.currentPhrase.isRest(reader.getNoteIndex())) {
                float notept = -1;
                ...
                float angle1 = PApplet.map(notept, 0, pa.currentPhrase.getTotalDuration(), 0, PApplet.TWO_PI);
                float angle2 = PApplet.map(notept + pa.currentPhrase.getSCDuration(noteIndex), 0,
                        pa.currentPhrase.getTotalDuration(), 0, PApplet.TWO_PI);
            ...
            dataPts.add(new DataPoint(spawnX, y1,
                    spawnX + PIXELS_PER_WHOLE_NOTE * pa.currentPhrase.getSCDuration(noteIndex), opacity));
        }
    }
     ...
     * @param dataPts
     *            The list of data points to translate.
     */
    ...
    /**
     * Fades away a list of data points by the fade rate, which is specified by
     * a helper method fadeAmt(dt).
     * 
     * @param dataPts
     *            The list of data points.
     * @param dt
     *            The number of milliseconds since the last update() invocation.
     */
     ...
     * @param dt
     *            The number of milliseconds since the last update() invocation.
     * @return The amount to decrease opacity by given that dt milliseconds have
     *         passed.
     */
     ...
     * 
     * @param dx
     *            The number of pixels to move the spawn point horizontally.
     * @param dy
     *            The number of pixels to move the spawn point vertically.
     */
     ...
     * 
     * @param noteIndex
     *            The index of the note to the Phrase.
    ...
    /**
     * A single note in the pitch/time plot of notes, with a start point, an end
     * point, an opacity, and an index to the Phrase.
     * 
     * @author James Morrow
     *
     */
    private class DataPoint {
         ...
         * 
         * @param startX
         *            The x-coordinate (an image of time) at which the note
         *            starts.
         * @param startY
         *            The y-coordinate (an image of pitch) of the note.
         * @param endX
         *            The x-coordinate (an image of time) at which the note
         *            ends.
         * @param opacity
         ...
         * 
         * @param dx
         *            The number of pixels to translate the note horizontally.
         */
