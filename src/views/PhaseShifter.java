...
/**
 * The PhaseShifter View type. It show two identical plots of notes that are
...
public class PhaseShifter extends View {
    ...
    /**
     * Constructs a PhaseShifter whose option values are taken from the another
     * PhaseShifter.
     * 
     * @param ps
     *            The PhaseShifter this one derives its option values from.
     * @param viewBox
     *            The area in which to draw.
     * @param opacity
     *            The opacity of notes.
    ...
    /**
     * Copies the given PhaseShifter object's option values into this
     * PhaseShifter object's option variables.
     * 
     * @param ps
     *            The given PhaseShifter.
     */
    ...
    /**
     * Initializes the variables that have to do with the boundaries within
    ...
    /**
     * Initializes note graphic positioning data and stores it so it doesn't
    ...
    /**
     * Updates the transformation variables, which can later be translated into
     * a rotation amount or a translation amount.
     * 
     * @param dNotept1
     *            The amount of time passed since the last update, in terms of
     *            how much music player 1 played.
     * @param dNotept2
     *            The amount of time passed since the last update, in terms of
     *            how much music player 2 played.
     */
     ...
     * Whether that's a translation or a rotation depends on the transformation
     * mode.
     * 
     * @param waveNum
     *            Specifies which wave (graph) is to be transformed.
     */
    private void transform(int waveNum) {
        float normalAcc = (waveNum == 2) ? normalTransform1 : normalTransform2;
        switch (transformation.toInt()) {
        case TRANSLATE:
            pa.translate(-normalAcc * width, 0);
            break;
        case ROTATE:
            pa.rotate(-normalAcc * pa.TWO_PI);
            break;
        }
    }
    ...
    private boolean showActiveNote() {
        return (activeNoteMode.toInt() == SHOW_ACTIVE_NOTE || activeNoteMode.toInt() == ONLY_SHOW_ACTIVE_NOTE);
    }
     ...
     * 
     * @param activeNote
     *            The index of the note (or rest) that is currently being
     *            played.
     * @param waveNum
     *            Specifies which wave (graph) is to be transformed.
     */
    ...
    private void drawActiveLineSegment(int waveNum, int activeNote, int nonActiveColor, int activeColor,
            float xOffset) {
     ...
     * 
     * @param normalTransform
     *            A normal value (one that's between 0 and 1) that determines a
     *            transformation (a translation in this case).
     */
    ...
    /**
     * Contains two adjacent data points. Used to draw lines between
     * connections.
     * 
     * @author James Morrow
     *
    ...
    /**
     * Container for the positions of note graphics. This is so the data doesn't
     * have to be recalculated every time its used, which is every frame.
     * 
     * @author James Morrow
     *
     */
    private class DataPoint {
        ...
        private DataPoint(int i) {
            ...
            ty = (pa.currentPhrase.minPitch() != pa.currentPhrase.maxPitch()) ? pa.map(pa.currentPhrase.getSCPitch(i),
                    pa.currentPhrase.minPitch(), pa.currentPhrase.maxPitch(), halfHeight, -halfHeight) : 0;
            ...
            radius = (pa.currentPhrase.minPitch() != pa.currentPhrase.maxPitch())
                    ? pa.map(pa.currentPhrase.getSCPitch(i), pa.currentPhrase.minPitch(), pa.currentPhrase.maxPitch(),
                            minRadius, maxRadius)
